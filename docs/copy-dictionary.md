...
| **Trading ends（加密快轮）** | 加密轮没有冻结期，`Trading ends` = 轮次结束时间（页头 ⓘ tooltip、手机 ⓘ、Event Info `Trading ends` 格同源 `freezeEtOnly`，crypto 回退到 `closeEtOnly`）；股票仍 = `freeze_time`（研发问题 #14，原来显示 `—`） | —, TBD |
| **Event Info（现货）六格** | `Trading ends（倒计时 · 时刻）/ Volume / Round open（股票为 Prior official close）/ Settles vs / Resolution source / Symbol` 各一格、单一来源；加密 Symbol `ETH · USD · 15m round`，股票 `META · Nasdaq`；没有 Event End Date / Total Volume / Open Interest（研发问题 #18） | Open Interest $1.2M, 两个 Volume, ETH · Nasdaq |
| **订单簿精度（无切换）** | 订单簿右上角不再有精度下拉；聚合固定原生 tick：合约 `0.0001`、现货 `0.01`（研发问题 #2，后端无更粗档位） | 0.001 / 0.01 / 0.1 / 1 档位, Precision |
