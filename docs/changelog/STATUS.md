...
| # | 项 | Status | Notes |
|---|---|---|---|
| ST-A | 页面骨架：ACCOUNT family，`max-w-7xl` 容器无标题，通栏 hero → 两卡带 → 8/4 栅格，SeoFooter；游客 LiteAuthGate；错误 ErrorState | ✅ | `Settings.tsx`；DESIGN §Addendum 2026-09-22；§4 Layout Narrow 废止 |
...
| ST-M | Authenticator 行显示启用日期 | ⬜ | `user_security` 不暴露时间 |
...
| SW-D | Lite 三种下单面板 CTA 下方**一行**脚注二选一：金额为空 `Want to place a limit order? Pro ›`，金额 > 0 原风险句 / 原句（桌面 + 手机抽屉）；入口只给该账号未进过 Pro 时看 | ⬜ | `LimitOrderHint`，localStorage `omenx_pro_visited:<uid>` |
| SW-E | 点击切 Pro、带事件 / market / 选边，Pro 面板 orderType 落在 Limit | ⬜ | sessionStorage `omenx_open_limit` 时间戳，15 s 内挂载的 Pro 面板都落 Limit |
| SW-F | 字典 TR-27 / TR-27b / SP-19 / SP-19b | ⬜ | |
...
| ES-E | 手机现货 header `SPOT` 标删除；合约页不加 `Boost` 标 | ⬜ | 终端 header 只留生命周期 badge |
| ES-F | 字典 ES-D1…ES-D4（桌面）、ES-M1（手机抽屉） | ⬜ | |
...
## 2026-09-18 — Affiliate Program 页面 视觉修订（[文档](../delivery/affiliate-page-v2.md)）
...
## 2026-09-14 — Affiliate Program 页面 v1（[文档](../delivery/affiliate-page-v2.md)）
...
- 2026-07-29 · R3b-2 round 7 — 赔率图跟随下单卡选中方向（No 序列 = 100 − Yes，颜色/标题/toggle 走 MARKET 轴，无重挂载）；`market_activity` 补 Pending→Filled 的 AFTER UPDATE 触发器（与 INSERT 触发器互斥不重复），行改为定宽 ledger 网格；LiteSection 去掉全部 "shared" chip、新增 Where things live 表与 375px 移动挂载 frame，标注标准写入 memory。[2026-07-29-r3b2-round7-chart-activity-styleguide.md]
...
- 2026-07-29 · R3b-2 round 15 — M1 收口：Lite 列表页移动端改回 logo-only preset A；补 Politics/Finance/Social 品类 pill（仍按有无 live 事件渲染）；新增 ★ Watchlist pill（未登录 → AuthDialog，空态引导）；间距改为 space-y 体系（视觉零变化）；Pro escape-hatch 文案登记为 forbidden-word 例外；style guide 补 "Markets list" 子节。[2026-07-29-r3b2-round15-m1-closure.md]
- 2026-08-31 · 批量平仓动作条改 fixed 贴底 — `BatchActionBar` 由 `sticky`（列表短时停在页中）改 `fixed inset-x-0 bottom-[84px] lg:bottom-4`（BottomNav 实测 77px 高含 1px border，bottom-[76px] 会压住 1px → 改 84px 留 7px 缝），恒定钉在移动端 BottomNav 上方；内层 `mx-auto max-w-7xl px-4 lg:px-6` 与页容器对齐；LitePortfolio 流内加 `h-[76px]` 占位防遮末行。视觉与逻辑不变。
