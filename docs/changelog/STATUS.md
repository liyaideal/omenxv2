...
- 2026-08-31 · 批量平仓动作条改 fixed 贴底 — `BatchActionBar` 由 `sticky`（列表短时停在页中）改 `fixed inset-x-0 bottom-[76px] lg:bottom-4`，恒定钉在移动端 BottomNav 上方；内层 `mx-auto max-w-7xl px-4 lg:px-6` 与页容器对齐；LitePortfolio 流内加 `h-[76px]` 占位防遮末行。视觉与逻辑不变。
