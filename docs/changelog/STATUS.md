...
- 2026-08-31 · Live 列表多选 + 全选批量平仓 — Lite /portfolio live 列表新增 Select 模式（行内勾选点、Select all / Clear / N selected / Cancel 工具条、吸底动作条 `Cash out N`），确认层桌面 Dialog / 移动 MobileDrawer，串行复用 `usePositions.closePosition`，进度 `Closing i / N…` + 部分失败汇总 toast；单行 Cash out 跳市场页不变，Pro 不涉及。Playground key `portfolio-lite-live-select`。
