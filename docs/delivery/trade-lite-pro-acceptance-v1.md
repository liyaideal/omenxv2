...
| H3 | 「两终端共用」（09-21 nav 整理后） | SS-1…SS-6 开关六态 | ✅ | |
...
| 5 | E2 | Liya（生产，09-21） | 限价单部分成交时 Current Orders 没有成交明细提示（桌面现货表 / 手机卡都没有；桌面合约表有个老 hover） | ✅ PF-1 已做（`OrderStatusBadge`，四处挂载，字典 OS-D1 / OS-M1（原 PF-D1 / PF-M1，09-21 改号））；生产上 `Partial Filled` 因引擎整单成交不可达，见 `order-status-partial-fill-v1.md` |
...
- Liya 按 30 条清单过了一遍：全部通过；验收中提出的部分成交明细（PF-1）、风险比例口径（RM-1）两轮已做完并在生产复验（三处百分比一致）。
- 剩余 ➖：G5 现货冻结灰条（等真冻结窗口，不阻塞）。
- **验收通过，进入交付。**
