...
> 这份文档讲 Lite 主页（网址 `/`，和 `/events` 是同一个页面）改版之后长什么样、怎么运转，给实现它的研发和测试看。
> 用户能看见的变化：页面最上面多了一条会滚动的行情带；标题区换成新的 hero；品类筛选条改成通栏；中间是四张卡组成的舞台——加密快轮、今日收盘股票、体育、编辑精选；股票卡会按「开市中 / 收盘后一小时 / 等下一场」三种状态自动切换文案和能不能点；移动端整页按手机重新排版。
> 没动的部分，研发不要碰：Pro 面、目录里的事件卡、徽标规则、目录排序、筛选条的内容和行为、页脚三行。
> 怎么读：先看 §0 知道去哪查，再看「通俗导读」建立概念，正文里每个数值都标了代码文件；找不到代码来源的写在 §12「待核实」。历史轮次代号与本文章节的对照见 §14。
...
| 代号 | 白话含义 | 本文章节 |
|---|---|---|
| HP-1 | 桌面主页改版：行情带、hero、筛选通栏、四卡舞台、目录板头 | §4.1 · §2.c · §2.d |
| HP-1d | 体育卡高度封顶（几场比赛都一样高） | §2.e |
| 空白修复 | 筛选条到第一张卡之间的间距收紧 | §4.1 / §4.2 间距值 |
| HP-3 / 3b / 3c | 移动端整页重排、插画位删除、股票行 Up/Down 按钮与加密卡统一 | §4.2 · §2.b |
| ST-1 / ST-1d | 股票交易时段三态状态机；每日种子按交易日生成 | §2.a · §2.b · §3 |
| SG-HP | style-guide 状态字典与几何总表 | §5 · §7 |
