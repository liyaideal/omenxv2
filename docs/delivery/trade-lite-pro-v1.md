...
| 步 | 做什么 | 看到什么 |
|---|---|---|
| 1 | 打开 `https://omenxv2.lovable.app/style-guide`（不用登录） | 左栏是节点树。本次交付只在四个节点里：**Pro — 交易终端** 组的 `合约终端 /trade` · `现货终端 /spot` · `两终端共用`，以及 **Lite** 组的 `交易页` |
| 2 | 点一个节点，先看顶部「本页目录」 | 目录 = 这个节点里的小节，按产品页从上到下排（① 页头 → ② 下单面板 → ③ dock → ④ 风险）；每条右侧是该小节里的编号范围（如 `CT-D1…6`）。点目录条直接滚过去 |
| 3 | 每个小节顶部一行黄框「定位行」 | 写着：这页对应哪个路由；字段 / 文案 / 公式去 `copy-dictionary.md` 哪一节；口径去哪份交付文档（现在就是本文） |
| 4 | 小节里每张样张 | 上面一行小字 = **编号 · 状态名（组件名）**，如 `CT-D3 · Buy · Limit 低于现价 → 挂单`；样张本体是 iframe 里的真组件（桌面帧 100% 宽、手机帧 375 宽），能 hover、能点、能滚 |
| 5 | 样张下面的表 | 列固定为「状态 / 触发条件（字段 · 公式）/ 视觉结果 / 数据来源」——**触发条件就是判定表达式**，来源列写的是字段名或规范坐标（`DESIGN §7` / `copy-dictionary §Trading`） |
| 6 | 找某个具体编号 | 页头搜索框直接敲编号（`SL-D2` / `TR-27b` / `OS-M1`）或文字，结果行「编号 · 名 → 节点 › 小节」，点了自动切节点并滚到那张样张（描一圈两秒） |
| 7 | 只要一张样张单独看 / 截图 | `https://omenxv2.lovable.app/style-guide/preview?c=<preview key>`，多张用逗号（key 在小节标题旁的 `1 iframe · N cases` 行不显示，问 PM 或看 `src/pages/StyleGuide/preview/registry.tsx`） |
