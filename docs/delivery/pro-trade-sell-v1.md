...
| 限价平仓 | 生成一张 **reduce-only** 挂单：`side = sell`、`order_type = Limit`、`margin = 0`、`fee = 0`、`reduce_only = true`，**下单不动余额**。Current Orders 表该行带 `Reduce-only` 标。撤单只改状态，无退款。mark 触及限价时按市价平仓同一条路径成交（蓝图侧为前端 touch-fill 模拟，见 §8）。 |
