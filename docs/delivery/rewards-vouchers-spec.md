...
一句话：**Trial Position Voucher = 平台出本金让你试开一单——赚了利润归你，亏了不用赔，券本身不是现金、不可提现**。`/rewards` 是活动、券、邀请的唯一入口。

```text
/rewards
├─ Campaigns  活动 + 任务（做任务 → 发券 / USDC）
├─ Vouchers   券的一生（领取 → 兑换开仓 → 结算入账）
└─ Referral   邀请
```

### 0.2 一条完整故事线（用户小A）

1. 小A 点开 KOL 发的专属链接 `?entry=CODE` 落地站点。
2. 系统把他绑定到该 campaign entry（未登录先暂存 code，登录后再判定）。
3. 他在活动详情页完成任务，达标后拿到一张券，状态 `granted`。
4. 他在 Vouchers tab 点 Tap to claim，券转 `claimed`。
5. 他进 redeem，挑一个符合条件的市场和方向（价格需落在 20¢–80¢ 价格带，一个事件只能用一张券），确认后券转 `redeemed`，试用仓开出。
6. 仓位最长持有 72h，到点自动平仓。
7. 利润按券类型入账：`instant` 直接进 Standard 钱包；`tiered` 先进待释放池，再按交易量阶梯解锁提取。

### 0.3 概念词典

| 概念 | 是什么 | 解决什么问题 | 一句话类比 |
|---|---|---|---|
| Trial Position Voucher | 一张能换成试用仓位的券 | 让新用户不动用自己资金体验一次交易 | 体验券 |
| Campaign | 一个有任务和奖励的活动 | 组织发放规则 | 活动本体 |
| Entry（专属链接） | campaign 下的一个带 code 的入口 | 区分不同渠道 / KOL 带来的人 | 活动的专属入口 |
| Grant 任务 | 活动里的单条任务及其进度与奖励 | 决定谁能拿到券 / USDC | 任务清单的一行 |
| instant / tiered | 券利润的两种入账方式 | 区分"直接到账"和"按量释放" | 直接打款 vs 分期释放 |
| voucher_earnings（释放池） | tiered 券利润的待领取余额 | 承接尚未解锁的利润 | 待领取账户 |
| 事件锁 | 同一事件只允许用掉一张券 | 防止单事件重复薅券 | 一事件一次机会 |

### 0.4 易混点辨析

1. **不是"券面值等于能拿走的钱"，而是"券是本金、利润才归你"**：最大利润 = `face_value × redeemable_cap_pct`（默认 0.5），实际到手 = `min(真实利润, 上限)`；亏了归零，不用赔。
2. **不是"发券和利润释放是同一个任务"，而是两套独立条件、独立记账**：发券看 campaign grant 进度；tiered 利润释放看交易量阶梯（T0–T4）。
3. **不是市场级锁，而是事件级锁**：one voucher per event —— 同一事件的 Boost 与 Standard 两条产品线一起锁。
4. **不是端规则，而是数据规则**：互补两选项市场折叠为方向对、真多选保留 per-option Yes/No —— 手机与桌面同款。
5. **不是"再点一个链接就能换归属"，也不是静默失败**：绑定后一旦 `locked_at`，再点别的链接不换归属；链接无效 / 不合格是当场拒入 + 跳回首页。
...
- 券面值、最大利润、持有窗口、券码
- 释放进度（tiered 券的待释放金额与阶梯）

用户看不到：

- `redeemable_cap_pct` 之类的字段名
- 渠道归因细节（entry / channel 的内部结构）
- 距结算最小时长、tier 解锁阈值等配置的字段名与内部参数（价格带数值本身会出现在空态与拒选文案里，属用户可见）
