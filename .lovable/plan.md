...
不新开文档。把上述内容并进原来那份，输出 **`/mnt/documents/lite-portfolio-spec-v2.md`**（v1 全文保留，章节顺序不变，只在后面续写）：

原有 §1–§10 保持原样，§10「未变更 / 不做」挪到最后，中间新增：

```
## 11. 前后端分工（前端算什么 / 后端需保证什么语义，不定义接口字段）
## 12. 异步态（loading 骨架 / error / 空态 三态表，含色值与重试 —— 待实现项）
## 13. 规模与分页（settled 分页口径、系列聚合下沉、仓位规模上限 —— 待实现项）
## 14. 实时刷新口径（频率、不可见降频、断线回落）
## 15. 时区与格式（切天/切月时区、金额精度、负零）
## 16. 权限（结算记录可见性与越权 URL 行为）
## 17. 埋点事件表
## 18. 无障碍要点
## 19. QA 验收 checklist（按 §3 四条流程逐条可勾选）
## 20. 待决策项（需后端 / QA 拍板的开放问题）
```

原则：**不改任何生产代码**；「原型没做」的（骨架屏、分页）明确标注为待实现项，不写成现状。
