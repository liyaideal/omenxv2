...
- **用词（2026-09-22）：** 这套站点叫 Lovable，不叫「蓝图」（文档 / 消息 / 回复统一）。
- **单一演示引擎:** 本项目的 Supabase 是所有 OmenX 前端 Lovable 项目（主站 / Sports / 未来 Pro / Lite）**唯一**后端；schema 变更只允许发生在本项目，其他前端只能调用现成 RPC / Edge Function。已有接入方：OmenX Sports（2026-07-13，两场 WC26 半决赛试点）。
...
- [Vouchers v2](mem://features/vouchers-v2) — 双收益模式（instant→钱包 / tiered→pending 池）、一券一 event 锁、入口并入 /rewards Vouchers tab
...
- [Footer compliance](mem://design/footer-compliance) — 底栏三行 FROZEN 文案、挂载覆盖面、切面入口口径、移动壳 footer breakout
