...
- **自动弹**：`LiteCashOutFlow` 平仓成功后自动打开分享流；全量平仓会卸载该 flow，故快照上抛由页面级 `LiteCashOutShareCard` 承载。合约（/trade）、日线现货、快速回合（LiteQuickTrade）三个交易页均已接入。
- **五入口**（均为可选 `onShare?`，不传 = 生产 DOM 零变化）：
  ...
  5. 快速回合持仓卡 — `LiteQuickTrade` 的 `SpotYourPosition`，未登录不渲染；方向行 `Up · 15m round` 形态。
