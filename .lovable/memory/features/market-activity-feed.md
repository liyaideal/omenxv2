...
- UI：`LiteMarketActivity`，**统一 ledger 行格式（2026-07-31 LOCKED，binary/spot/multi 完全一致）**：`[相对时间 | "Backed {side}"（MARKET 轴配色）| muted context | $金额右对齐]`。唯一差别是 context 列：binary/spot = `{N}×`（只有 boost，页面本身就是这个市场），multi = `{option} · {N}×`。旧的「side chip 打头 + 时间收尾」布局已废弃，禁止回退。side 以 `market_activity.is_yes` 为准，`"No: "` 前缀仅作 legacy 兜底。空态 "No activity yet"。
