...
const PORTFOLIO_MOBILE_CASES: SectionCase[] = [
  ...
  },
  {
    ...
    spec: [
      ...
      { state: "动作条（N≥1）", when: "selectedRows.length > 0", visual: "fixed 贴底圆角条（移动端钉在 BottomNav 上方 bottom-[76px]，桌面 lg:bottom-4，内层 max-w-7xl px-4 lg:px-6 与页容器对齐；页面流内同步渲染 h-[76px] 占位防遮末行）：`N selected` + `Now worth $X · Profit ±$Y` + 主按钮 `Cash out N`；N=0 不渲染", source: "BatchActionBar / LitePortfolio" },
