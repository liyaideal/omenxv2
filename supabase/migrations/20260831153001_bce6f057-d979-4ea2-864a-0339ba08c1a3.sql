  ...
  IF FOUND THEN
    ...
    ELSE
      ...
      IF maps_h >= need OR maps_a >= need THEN
        ...
        -- siblings: clear last cycle's residue so it cannot accumulate
        UPDATE public.events
          SET start_date = sd, end_date = ed,
              is_resolved = false, settled_at = NULL, winning_option_id = NULL,
              lifecycle_status = 'TRADING',
              metadata = (coalesce(metadata,'{}'::jsonb) - 'score')
                || jsonb_build_object('score', NULL)
                || jsonb_build_object(
                  'kickoff_at', to_char(sd at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"')),
              updated_at = now()
          WHERE metadata->>'fixture_id' = 'demo-live-cs2' AND id <> 'demo-live-cs2';

        UPDATE public.event_options o
          SET is_winner = false, final_price = NULL, updated_at = now()
          FROM public.events e
          WHERE o.event_id = e.id
            AND e.metadata->>'fixture_id' = 'demo-live-cs2'
            AND e.id <> 'demo-live-cs2';
