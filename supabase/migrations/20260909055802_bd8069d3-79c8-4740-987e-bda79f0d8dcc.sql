REVOKE EXECUTE ON FUNCTION public.settle_futures_event(text) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.settle_futures_sweep() FROM anon, authenticated;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    v_pnl := round(GREATEST(v_pnl, -r.margin), 2);

    -- Winning commission + cash return use the SAME rule as settle_futures_event.
    SELECT COALESCE(t.fee, 0) INTO v_fee FROM public.trades t WHERE t.id = r.trade_id;
    v_fee := COALESCE(v_fee, 0);
    v_wc := CASE WHEN v_pnl > 0 THEN round(0.05 * GREATEST(v_pnl - v_fee, 0), 2) ELSE 0 END;
    v_cash := COALESCE(r.margin, 0) + v_pnl - v_wc;
    ...
    UPDATE public.positions
       SET status = 'Closed',
           ...
           winning_commission = COALESCE(winning_commission, 0)
             + CASE WHEN r.product_line IN ('futures','contract') THEN v_wc ELSE 0 END,
           closed_at = now(),
           updated_at = now()
    ...
    IF r.trade_id IS NOT NULL THEN
      UPDATE public.trades
         SET status = 'Closed', pnl = v_pnl, closed_at = now(), updated_at = now()
       WHERE id = r.trade_id;
    END IF;

    IF r.product_line IN ('futures','contract') THEN
      ...
      VALUES (r.user_id,
              ...
              v_pnl, 'futures',
              'Settled: ' || r.event_name || ' · ' || r.option_label || ' · ' ||
              CASE WHEN v_pnl >= 0 THEN 'Won' ELSE 'Lost' END,
              'completed');

      IF v_wc > 0 THEN
        INSERT INTO public.transactions (user_id, type, amount, account, description, status)
        VALUES (r.user_id, 'winning_commission', -v_wc, 'futures',
                'Winning commission · 5% · ' || r.option_label || ' · ' || r.event_name,
                'completed');
      END IF;
    END IF;
  ...
  LOOP
    ...
    VALUES (v_alex, ev.name, r.label, CASE WHEN v_side = 'long' THEN 'buy' ELSE 'sell' END,
            ...
            v_size, v_lev::int, v_margin, round(v_margin * v_lev * 0.0015, 4), 'Filled', 'futures')
  ...
  LOOP
    ...
    WHERE e.is_resolved = false
      ...
      AND e.event_subtype IS DISTINCT FROM 'CRYPTO_QUICK_UPDOWN_SPOT'
      AND (e.lifecycle_status IS NULL OR e.lifecycle_status IN ('ACTIVE','TRADING','EXTENDED_TRADING'))
