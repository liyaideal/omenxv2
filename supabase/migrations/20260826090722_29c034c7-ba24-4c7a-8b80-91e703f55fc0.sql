ALTER TABLE public.position_vouchers
  ADD COLUMN IF NOT EXISTS tiered_credited_at timestamptz;

CREATE OR REPLACE FUNCTION public.credit_tiered_voucher_settlement()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NEW.source <> 'voucher' THEN RETURN NEW; END IF;

  SELECT id, user_id, payout_mode, tiered_credited_at
    INTO v
  FROM public.position_vouchers
  WHERE redeemed_airdrop_position_id = NEW.id
    AND payout_mode = 'tiered'
    AND tiered_credited_at IS NULL
  LIMIT 1;
  ...
  UPDATE public.position_vouchers
     SET tiered_credited_at = now(), updated_at = now()
   WHERE id = v.id AND tiered_credited_at IS NULL;
  IF NOT FOUND THEN RETURN NEW; END IF;

  v_amount := COALESCE(NEW.settled_pnl, 0);
  IF v_amount <= 0 THEN RETURN NEW; END IF;

  INSERT INTO public.voucher_earnings (user_id, pending_amount, lifetime_credited, last_settled_at)
  VALUES (v.user_id, v_amount, 0, now())
  ON CONFLICT (user_id) DO UPDATE
    SET pending_amount = public.voucher_earnings.pending_amount + EXCLUDED.pending_amount,
        last_settled_at = now(),
        updated_at = now();

  INSERT INTO public.voucher_earnings_ledger (user_id, type, amount, airdrop_position_id, description)
  VALUES (v.user_id, 'accrual', v_amount, NEW.id,
          'Voucher position settled (' || COALESCE(NEW.close_reason, 'EVENT_RESOLVED') || ')');

  RETURN NEW;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.credit_tiered_voucher_settlement() FROM PUBLIC, anon, authenticated;

DROP TRIGGER IF EXISTS trg_credit_tiered_voucher_settlement ON public.airdrop_positions;
CREATE TRIGGER trg_credit_tiered_voucher_settlement
AFTER UPDATE OF status ON public.airdrop_positions
FOR EACH ROW EXECUTE FUNCTION public.credit_tiered_voucher_settlement();
...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_alex, ev.name, r.label, CASE WHEN v_side = 'long' THEN 'buy' ELSE 'sell' END,
            'Market', v_entry, round(v_margin * v_lev, 2),
  ...
  LOOP
    ...
    INSERT INTO public.trades (user_id, event_name, option_label, side, order_type, price, amount,
                               quantity, leverage, margin, fee, status, product_line)
    VALUES (v_alex, ev.name, r.label, 'buy', 'Market', v_entry, v_margin,
