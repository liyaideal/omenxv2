...
2. **Inline accordion chart** (`LiteBoardChart.tsx`) opens under the selected row only. Desktop only by default; mobile passes `showChart={false}` and uses `LiteCrowdOverview` as the summary. **Row→chart boundary carries NO divider line** (2026-07-31: the dashed seam is deleted — it rendered as a bright dotted line). The boundary is expressed only by (a) the shared `--yes/55` side borders continuing from row into panel, (b) the darker panel background `#0C1216` against the row's card bg, and (c) ~6px extra top padding on the panel. Same on compact/mobile.
