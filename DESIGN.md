...
- 禁止 `px-8` 外层水平内边距（已废止，统一 `px-4 lg:px-6`）
- 禁止 `px-6` / `py-8` / `p-6` 作为外层容器节奏
- 禁止内容整列 `text-center` 居中

### §Addendum 2026-07-30 · Two page families (LOCKED)
...
price in `font-mono font-bold shrink-0`, label `truncate`. Only **unpriced** labels
...
**Sports game lines（2026-08-17）**：一场比赛由多条兄弟事件组成，共享 `metadata.fixture_id`，用 `metadata.market_type` 区分 `winner` / `handicap` / `total`，`metadata.line` 存线值（handicap 以主队视角带符号，负号用真减号 U+2212；total 为正数）。交易页把它们**分组渲染在同一块 `LiteMarketBoard` 上**：组标题（Winner / Handicap / Total goals｜Total points）+ 每个线组一行；行内 chip 文案直接取该兄弟事件的 `side_labels`（如 `ARS +1.5` / `LIV −1.5`、`Over 2.5` / `Under 2.5`），价格与配色规则不变；strip 下方挂 `LiteLineScrubber` 做**离散换线**（不是滑杆，无拖拽惯性），换线只是把该组绑定到另一条兄弟事件。无 `fixture_id` 的事件渲染路径与 DOM 完全不变。
...
| 2. 配置 | `/settings/api` | Key 自助管理 | §4 Layout Wide + `<PageHeader>` 标题块（`max-w-7xl mx-auto px-4 lg:px-6 py-10`），去程靠 `/developers` 的 CTA，无正文面包屑 |
...
> **废止注（2026-09-21）**：下面这段锁定的现货页头规格已被 spot-pro-v1.md §（页头）与 event-selector-v1.md 取代：页头不再有 SPOT 徽标与 Base 统计，时间不带时区，`Credits by` 改为 `Payout by ~{time}`，产品身份只在事件选择器的 Standard / Boost 页签。以字典「现货终端 /spot」节点为准。
...
1. **Market board** (`src/components/lite/multi/LiteMarketBoard.tsx`) replaces both the sentiment bar and the standalone chart. One row per option: label (semibold) · optional "You hold" chip · chance % · independent `Yes {c}¢` / `No {c}¢` chips · 4px dual-tone strip (Pulse Blue Yes / Volt Green No). **Row order (LOCKED):** live options keep their natural event order; **all settled options sink below every live option**, keeping their own relative order. List cards and `LiteCrowdOverview` are unaffected.
2. **Inline accordion chart** (`LiteBoardChart.tsx`) opens under the selected row only, on **both** desktop and mobile — `showChart` is unconditional; the mobile variant is the compact chart (~150px). `LiteCrowdOverview` is an **additional** read-only sentiment summary rendered above the board on mobile, never a chart replacement. **Row→chart boundary carries NO divider line** (2026-07-31: the dashed seam is deleted — it rendered as a bright dotted line). The boundary is expressed only by (a) the shared `--yes/55` side borders continuing from row into panel, (b) the darker panel background `#0C1216` against the row's card bg, and (c) ~6px extra top padding on the panel. Same on compact/mobile.
...
5. **Per-option netting (shipped):** buying the opposite side of an option you hold nets it down first and may **FLIP** (the remainder opens the new side) in one `balanceDelta` — the interim `blockNotice` guard is gone. Notice copy: "You're backing {side}. This cashes out your {opposite} on this market first." Partial nets also disclose "Est. auto-close (new position)" for the remainder leg only.
6. **List cards (FROZEN anatomy, identical for every multi event):** no "Live" badge anywhere; badges follow the two-track system below. The preview shows the **top-2 options by yes-price** plus a `+N markets` footer. Each preview row is **ONE continuous `rounded-[8px]` row**, background `hsl(var(--yes)/0.05)`, with an absolute left-anchored fill layer behind the content at `width = yes-price %` in `hsl(var(--yes)/0.09)`; the option label is **plain semibold, left** (no box, no dark segment) and the bold `%` sits **right in `text-yes`**. Nothing else — no split segments, no standalone pill/track bars. Every multi card renders identically.
7. **Activity ledger (UNIFIED, all Lite surfaces):** `LiteMarketActivity` uses ONE row renderer in the frozen column order `[relative time | "Backed {side}" coloured on the MARKET axis | muted context | $amount right-aligned]` for binary contract, spot (alias Up/Down) and multi feeds alike. The only surface difference is the context column: binary = `{N}×` (boost only — the page IS the market), multi = `{option} · {N}×`. The old leading side-chip / trailing-time binary layout is abolished.
...
**2026-08-17 Sports Game Lines v1** — fixture 兄弟事件分组（winner / handicap / total）、`LiteLineScrubber` 离散换线、chip 文案取 side_labels；Lite 禁词新增 Spread(s) / Totals / 1X2 / O/U。
...
## §Addendum 2026-09-10 · SP-2-FIX2 · 移动现货零截断（append-only，补 §14）
...
## §Addendum 2026-09-15 · Pro 下单面板复审四则（CT-1 收口，append-only）
...
- 禁止：6 格验证码步、旧密码框、24 小时禁提现句（Lovable 无机制）、取消等待动作。
...
裁定（CPO 2026-09-22 批）：**判为「稿与 §4 全站容器规范冲突」，按 40 落**。不为单页开「逐页自定底部留白」的口子；稿上的 137 是设计侧的页面收尾节奏，不进代码。

**8 · 本页的比对基准与重叠关系（钉死，任何人改这页前先读）**
- **比对基准 = 页面顶端 0**，对照表第 1 行是顶栏本身。稿的 Y 从 frame 顶量，frame 顶 = 顶栏顶。拿 hero 顶或第一张卡当基准会让整张表"每行都对"而页面整体偏 65（2026-09-22 两次打回的根因）。
- **desktop 顶栏浮在头图上**：nav 0–73 与 hero 1–385 相交，顶栏 `sticky top-0 z-50 backdrop-blur`，头图靠 `marginTop: -65`（`EventsDesktopHeader` 实测 h-16 + 1px）铺到页顶。**不是上下排布。**
- **mobile 顶栏与头图上下排布**，顶栏 0–57、头图 57–170，无重叠。双端形态不同，不要互相套用。
- 复验基准值（2026-09-22 实测，稿 / 生产）：desktop 筛选行 430–478 / 430–478，领奖台占版 526–977 / 526–977（实渲收底 1204），渐变带 815–1051 / 815–1051，榜单卡 1041–1707 / 1041–1710，Your Ranking 1731–1819 / 1734–1822；mobile 筛选组 186–282、小标题 298–318、Top3 占版 334–494、榜单卡 510–1254、Your Ranking 1270–1516，生产逐项 0 ~ +4。±1–3 的差全部来自 `.trading-card` 的 1px 上下描边，属落准。

**9 · 稿与全站排版规范的两处冲突（按仓库 token 落，不改稿）**
- **移动顶栏标题居中基准**：稿把 "Leaderboard" 居中在返回钮与分享钮之间的 286px 容器（中心 x=190），生产居中在整页（中心 x=195），字宽一致（83 / 82.5）。按生产的整页居中落，与全站 Mobile Header System v1 一致。
- **`Overall Ranking` 小标题宽度**：稿 105，生产 100。差值来自 `font-display` 的 `-0.28px` 字距（全站字体 token）。按 token 落。
- **mobile 设计稿宽度是 390 不是 375**：本页 mobile 帧 711:33583 为 390×1629，横向比例核对一律按 390 取基准；字典帧沿用 375 只影响展示，不作为比对基准。

## §Addendum 2026-09-23 · Settings Close account 弹窗 = Connect modal 壳（LOCKED）
...
4. **进度文案**：`$value / $nextTarget`，千分位；分母恒为下一个未达档，全达后为最高档。桌面有标尺时计数与条顶对齐（`items-start` + `-mt-1.5`）。
4b. **档位标尺（桌面，2026-09-26 Liya 批）**：阶梯行桌面进度条加宽到 **360px**（直达 / 周期行仍 280），条下 5px 一条 12px 高的标尺，每个刻度点正下方写该档门槛：10px mono tabular，缩写 K 且**不带 `$`**（`2K / 2.5K / 30K / 200K`，单位由右侧计数承担；计数指标 `1 / 3 / 10`）；已达 / 已发 `#F2F3F5`，**下一档（第一个未达档）`#33D6FF` 700**，未达 `#6B7280`；全达 / 冻结无青。标签居中于点，最后一档右锚（`-translate-x-full`）不越出条尾；8 档间距 45px，最长标签 `100K` ≈ 24px。标尺只写门槛不写奖励——每档奖励在奖励槽行 1（下一档）与 tooltip。**移动端无标尺**（375 下无余量，档位全表走行 2 抽屉）。行高 74 → 88。
...
2. **K 线序列切换**（同日 Liya 改）：不叠线。周期选择器右侧放两段切换（与周期组同一套 `bg-muted/50 rounded-md p-0.5` 壳、选中 `bg-muted text-foreground`）：合约 `Last | Mark` 选蜡烛数据源；涨跌市场 `{选边} | {ticker} price` 选份额 / 标的视图，标的视图纵轴列 68px、白色 45% 虚线 `6,4` 的 `Base $X` 参考线 + 左侧小标签。手机（<768）切换独占周期行下一行、左对齐，图表容器 +40px。
